...
export class PedidosController {
  ...
  async getAll() {
    ...
    pedido = await Promise.all(pedido.map(async (elem) => {
      const itens = await this.itemPedido.getByOrder(elem.id);
      elem["itens"] = itens;
      const cliente = await this.clientes.getById(elem.customer);
    try{
  ...
  async update(@Param('id') id: number, @Body() pede: JSON) {
    const itens = pede["itens"];
    delete pede["itens"];
    const pedido = await this.pedidoService.update(id, pede);
    itens.map(elem => {
      const id = elem.id;
